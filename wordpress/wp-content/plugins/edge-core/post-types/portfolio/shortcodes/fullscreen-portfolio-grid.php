...
class FullscreenPortfolioGrid implements Lib\ShortcodeInterface {
	...
	public function __construct() {
		$this->base = 'edgtf_fullscreen_portfolio_grid';
		
		add_action( 'vc_before_init', array( $this, 'vcMap' ) );
		
		//Portfolio category filter
		add_filter( 'vc_autocomplete_edgtf_fullscreen_portfolio_grid_category_callback', array( &$this, 'portfolioCategoryAutocompleteSuggester', ), 10, 1 ); // Get suggestion(find). Must return an array
		
		//Portfolio category render
		add_filter( 'vc_autocomplete_edgtf_fullscreen_portfolio_grid_category_render', array( &$this, 'portfolioCategoryAutocompleteRender', ), 10, 1 ); // Get suggestion(find). Must return an array
		
		//Portfolio selected projects filter
		add_filter( 'vc_autocomplete_edgtf_fullscreen_portfolio_grid_selected_projects_callback', array( &$this, 'portfolioIdAutocompleteSuggester', ), 10, 1 ); // Get suggestion(find). Must return an array
		
		//Portfolio selected projects render
		add_filter( 'vc_autocomplete_edgtf_fullscreen_portfolio_grid_selected_projects_render', array( &$this, 'portfolioIdAutocompleteRender', ), 10, 1 ); // Render exact portfolio. Must return an array (label,value)
		
		//Portfolio tag filter
		add_filter( 'vc_autocomplete_edgtf_fullscreen_portfolio_grid_tag_callback', array( &$this, 'portfolioTagAutocompleteSuggester', ), 10, 1 ); // Get suggestion(find). Must return an array
		
		//Portfolio tag render
		add_filter( 'vc_autocomplete_edgtf_fullscreen_portfolio_grid_tag_render', array( &$this, 'portfolioTagAutocompleteRender', ), 10, 1 ); // Get suggestion(find). Must return an array
	}
	...
	public function vcMap() {
		if ( function_exists( 'vc_map' ) ) {
			vc_map( array(
					'name'     => esc_html__( 'Edge Fullscreen Portfolio Grid', 'edge-core' ),
					'base'     => $this->getBase(),
					...
					'params'   => array(
						...
						array(
							...
							'heading'     => esc_html__( 'Number of Portfolios', 'edge-core' ),
							'description' => esc_html__( 'Set number of items. Enter -1 to show all.', 'edge-core' ),
							'value'       => '-1'
						),
						array(
							'type'        => 'autocomplete',
							'param_name'  => 'category',
							'heading'     => esc_html__( 'One-Category Portfolio Grid', 'edge-core' ),
						...
						array(
							'type'        => 'autocomplete',
							'param_name'  => 'tag',
							'heading'     => esc_html__( 'One-Tag Portfolio Grid', 'edge-core' ),
	...
	public function render( $atts, $content = null ) {
		...
		$html = edgtf_core_get_cpt_shortcode_module_template_part( 'portfolio', 'fullscreen-grid-holder', '', $params, $additional_params );
	...
	public function getHolderClasses( $params ) {
		...
		switch ( $number_of_columns ):
			case '1':
				$classes[] = 'edgtf-fpg-one-column';
				break;
			case '2':
				$classes[] = 'edgtf-fpg-two-columns';
				break;
			case '3':
				$classes[] = 'edgtf-fpg-three-columns';
				break;
			case '4':
				$classes[] = 'edgtf-fpg-four-columns';
				break;
			case '5':
				$classes[] = 'edgtf-fpg-five-columns';
				break;
			default:
				$classes[] = 'edgtf-fpg-four-columns';
				break;
