...
class Banner implements Lib\ShortcodeInterface {
				...
				array(
					...
					'params'                    => array(
						...
						array(
							...
							'heading'    => esc_html__( 'Image Overlay Color', 'edge-core' )
						),
						array(
							'type'        => 'dropdown',
							'param_name'  => 'hover_behavior',
							'heading'     => esc_html__( 'Hover Behavior', 'edge-core' ),
							'value'       => array(
								esc_html__( 'Visible on Hover', 'edge-core' )   => 'edgtf-visible-on-hover',
								esc_html__( 'Visible on Default', 'edge-core' ) => 'edgtf-visible-on-default',
								esc_html__( 'Disabled', 'edge-core' )           => 'edgtf-disabled'
							),
							'save_always' => true
						),
						array(
							...
							'heading'     => esc_html__( 'Info Position', 'edge-core' ),
							'value'       => array(
								esc_html__( 'Default', 'edge-core' )  => 'default',
								esc_html__( 'Centered', 'edge-core' ) => 'centered'
							),
							'save_always' => true
						),
						array(
							...
							'heading'     => esc_html__( 'Info Content Padding', 'edge-core' ),
							'description' => esc_html__( 'Please insert padding in format top right bottom left', 'edge-core' )
						),
						array(
							'type'       => 'textfield',
							'param_name' => 'subtitle',
						...
						array(
							...
							'heading'    => esc_html__( 'Link Text Color', 'edge-core' ),
							'dependency' => array( 'element' => 'link', 'not_empty' => true )
						),
						array(
							'type'       => 'textfield',
							'param_name' => 'link_top_margin',
							'heading'    => esc_html__( 'Link Text Top Margin (px)', 'edge-core' ),
							'dependency' => array( 'element' => 'link', 'not_empty' => true )
						)
					)
				)
	...
	private function getLinkStyles( $params ) {
		...
		if ( ! empty( $params['link_top_margin'] ) ) {
			$styles[] = 'margin-top: ' . aalto_edge_filter_px( $params['link_top_margin'] ) . 'px';
		}
