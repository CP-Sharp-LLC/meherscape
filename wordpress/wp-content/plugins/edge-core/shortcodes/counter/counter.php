...
class Counter implements Lib\ShortcodeInterface {
				...
				array(
					...
					'params'                    => array(
						...
						array(
							...
							'value'       => array(
								esc_html__( 'Zero Counter', 'edge-core' )   => 'edgtf-zero-counter',
								esc_html__( 'Random Counter', 'edge-core' ) => 'edgtf-random-counter'
							),
							'save_always' => true,
						),
						array(
	...
	private function getCounterStyles( $params ) {
		...
		if ( ! empty( $params['digit_font_size'] ) ) {
			$styles[] = 'font-size: ' . aalto_edge_filter_px( $params['digit_font_size'] ) . 'px';
		}
