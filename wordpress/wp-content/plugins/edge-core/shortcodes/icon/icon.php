...
class Icon implements Lib\ShortcodeInterface {
		    ...
		    vc_map(
		    	array(
				    ...
				    'params'                    => array_merge(
					    ...
					    \AaltoEdgeIconCollections::get_instance()->getVCParamsArray(),
					    array(
						    array(
							    'type'       => 'dropdown',
							    'param_name' => 'size',
							    ...
							    'value'      => array(
								    ...
								    esc_html__( 'Huge', 'edge-core' )   => 'edgtf-icon-huge'
							    )
						    ),
						    array(
							    'type'       => 'textfield',
	...
	private function generateIconStyles( $params ) {
		...
		if ( ( $params['type'] !== 'edgtf-normal' && ! empty( $params['shape_size'] ) ) || ( $params['type'] == 'edgtf-normal' ) ) {
			if ( ! empty( $params['custom_size'] ) ) {
				$iconStyles[] = 'font-size:' . aalto_edge_filter_px( $params['custom_size'] ) . 'px';
			}
		}
	...
	private function generateIconHolderStyles( $params ) {
		...
		if ( $params['type'] !== 'edgtf-normal' ) {
			...
			if ( ! empty( $shapeSize ) ) {
				$iconHolderStyles[] = 'width: ' . aalto_edge_filter_px( $shapeSize ) . 'px';
				$iconHolderStyles[] = 'height: ' . aalto_edge_filter_px( $shapeSize ) . 'px';
				$iconHolderStyles[] = 'line-height: ' . aalto_edge_filter_px( $shapeSize ) . 'px';
			}
			...
			if ( ! empty( $params['border_color'] ) && ( isset( $params['border_width'] ) && $params['border_width'] !== '' ) ) {
				...
				$iconHolderStyles[] = 'border-width: ' . aalto_edge_filter_px( $params['border_width'] ) . 'px';
			...
			if ( $params['type'] == 'edgtf-square' ) {
				if ( isset( $params['border_radius'] ) && $params['border_radius'] !== '' ) {
					$iconHolderStyles[] = 'border-radius: ' . aalto_edge_filter_px( $params['border_radius'] ) . 'px';
				}
			}
		}
