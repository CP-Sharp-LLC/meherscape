	...
	function edgtf_core_add_image_gallery_attachment_custom_field( $form_fields, $post = null ) {
		if ( wp_attachment_is_image( $post->ID ) ) {
			...
			$form_fields['image_gallery_masonry_image_size'] = array(
				...
				'helps' => esc_html__( 'Choose image size for Image Gallery shortcode item - Masonry layout', 'edge-core' )
			);
			
			$form_fields['image_gallery_masonry_image_size']['html'] = "<select name='attachments[{$post->ID}][image_gallery_masonry_image_size]'>";
			$form_fields['image_gallery_masonry_image_size']['html'] .= '<option ' . selected( $field_value, 'edgtf-default-masonry-item', false ) . ' value="edgtf-default-masonry-item">' . esc_html__( 'Default Size', 'edge-core' ) . '</option>';
			$form_fields['image_gallery_masonry_image_size']['html'] .= '<option ' . selected( $field_value, 'edgtf-large-masonry-item', false ) . ' value="edgtf-large-masonry-item">' . esc_html__( 'Large Size', 'edge-core' ) . '</option>';
