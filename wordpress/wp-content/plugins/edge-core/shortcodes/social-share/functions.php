...
if ( ! function_exists( 'aalto_edge_get_social_share_html' ) ) {
	/**
	 ...
	 * @return mixed|string
	 */
	function aalto_edge_get_social_share_html( $params = array() ) {
		if ( aalto_edge_core_plugin_installed() ) {
			return aalto_edge_execute_shortcode( 'edgtf_social_share', $params );
		}
	}
}

if ( ! function_exists( 'aalto_edge_the_excerpt_max_charlength' ) ) {
	/**
	 ...
	 */
	function aalto_edge_the_excerpt_max_charlength( $charlength ) {
		$twitter_via_meta = aalto_edge_options()->getOptionValue( 'twitter_via' );
