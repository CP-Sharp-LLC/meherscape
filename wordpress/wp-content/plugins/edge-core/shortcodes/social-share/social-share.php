...
class SocialShare implements Lib\ShortcodeInterface {
	...
	public function render( $atts, $content = null ) {
		...
		$params['enable_social_share'] = ( aalto_edge_options()->getOptionValue( 'enable_social_share' ) == 'yes' ) ? true : false;
		
		//Is social share enabled for post type
		$post_type         = get_post_type();
		$params['enabled'] = ( aalto_edge_options()->getOptionValue( 'enable_social_share_on_' . $post_type ) == 'yes' ) ? true : false;
	...
	private function getSocialNetworksParams( $params ) {
		...
		foreach ( $this->socialNetworks as $net ) {
			...
			if ( aalto_edge_options()->getOptionValue( 'enable_' . $net . '_share' ) == 'yes' ) {
				...
				$params['custom_icon'] = ( aalto_edge_options()->getOptionValue( $net . '_icon' ) ) ? aalto_edge_options()->getOptionValue( $net . '_icon' ) : '';
    ...
    private function getSocialNetworkShareLink($net, $image) {
        switch ($net) {
            ...
            case 'twitter':
                ...
                $twitter_via = (aalto_edge_options()->getOptionValue('twitter_via') !== '') ? ' via ' . aalto_edge_options()->getOptionValue('twitter_via') . ' ' : '';
                if (wp_is_mobile()) {
                    $link = 'window.open(\'https://twitter.com/intent/tweet?text=' . urlencode(aalto_edge_the_excerpt_max_charlength($count_char) . $twitter_via) . get_permalink() . '\', \'popupwindow\', \'scrollbars=yes,width=800,height=400\');';
                } else {
                    $link = 'window.open(\'http://twitter.com/home?status=' . urlencode(aalto_edge_the_excerpt_max_charlength($count_char) . $twitter_via) . get_permalink() . '\', \'popupwindow\', \'scrollbars=yes,width=800,height=400\');';
                }
                break;
            case 'google_plus':
            ...
            case 'tumblr':
                ...
                break;
            case 'pinterest':
                $link = 'popUp=window.open(\'http://pinterest.com/pin/create/button/?url=' . urlencode(get_permalink()) . '&amp;description=' . aalto_edge_addslashes(get_the_title()) . '&amp;media=' . urlencode($image[0]) . '\', \'popupwindow\', \'scrollbars=yes,width=800,height=400\');popUp.focus();return false;';
                break;
            case 'vk':
