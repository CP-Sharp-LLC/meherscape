...
class EdgeTwitterApi {
    ...
    public function obtainRequestToken() {
        ...
        if(is_wp_error($response)) {
            ...
            $responseObj->message = esc_html__('Internal WP error', 'edgtf');
        } else {
            ...
            if(!empty($responseBody)) {
                ...
                if((is_array($responseParsed) && count($responseParsed)) &&
                    ...
                    if(!empty($responseObj->redirectUrl)) {
                        $responseObj->status = false;
                        $responseObj->message = esc_html__('Redirect URL couldn\t not be generated', 'edgtf');
                    } else {
                    ...
                    $responseObj->status = false;
                    $responseObj->message = esc_html__('Couldn\'t connect with Twitter API', 'edgtf');
                }
            }
        }
        ...
        if(!empty($authorizeVerifier) && !empty($authorizeToken)) {
            ...
            if(is_wp_error($response)) {
                ...
                if(is_array($responseParsed) && count($responseParsed)
                    ...
                    $responseObj->status = true;
                    $responseObj->message = esc_html__('Access token obtained', 'edgtf');
                }
            }
        } else {
            $responseObj->status = false;
            $responseObj->message = esc_html__('Authorize token and it\'s secret were not obtainer', 'edgtf');
        }
    ...
    public function fetchTweets($userId = '', $count = '', $transient = array()) {
        ...
        if(!$this->transientExists($transient)) {
            if($userId && $accessToken) {
                ...
                if(is_wp_error($response)) {
                    ...
                    if(isset($response['response']['code']) && $response['response']['code'] == 200) {
                        ...
                        $responseObj->status = false;
                        $responseObj->message = esc_html__('Couldn\'t connect with Twitter', 'edgtf');
                    }
                }
            } else {
                $responseObj->status = false;
                $responseObj->message = esc_html__('It seams like you haven\t connected with your Twitter account', 'edgtf');
            }
        } else {
            ...
            if(!empty($transientContent)) {
                ...
                $responseObj->status = false;
                $responseObj->message = esc_html__('Couldn\'t retreive content from database', 'edgtf');
            }
        }
