    ...
    <div class="edgtf-tl-wrapper edgtf-outer-space">
            ...
            <?php if ( is_array( $response->data ) && count( $response->data ) ) { ?>
                <ul class="edgtf-twitter-list">
                    <?php foreach ( $response->data as $tweet ) { ?>
                        <?php
                            $params['tweet'] = $tweet;
                            echo edgtf_twitter_get_shortcode_module_template_part('templates/item', 'twitter-list', $params['type'], $params);
                        ?>
                    <?php } ?>
                </ul>
            <?php } else { ?>
        ...
        <?php } else { ?>
            <div class="edgtf-twitter-not-connected">
                <?php esc_html_e( 'It seams that you haven\'t connected with your Twitter account', 'edge-twitter-feed' ); ?>
            </div>
        <?php } ?>
    </div>
</div>