...
class EdgeTwitterWidget extends WP_Widget {
	...
	public function widget( $args, $instance ) {
		...
		$skin = ! empty( $skin ) ? $skin : 'dark';
		
		$holder_classes = 'edgtf-twitter-' . esc_attr( $type_of_widget );
		$holder_classes .= ' edgtf-twitter-' . esc_attr( $skin );
		$slider_data    = array();
		...
		if ( $twitter_api->hasUserConnected() ) {
			...
			if ( $response->status ) {
				if ( is_array( $response->data ) && count( $response->data ) ) { ?>
					<ul class="edgtf-twitter-widget <?php echo esc_attr( $holder_classes ); ?>" <?php echo aalto_edge_get_inline_attrs( $slider_data ); ?>>
						<?php foreach ( $response->data as $tweet ) { ?>
							
							<?php if ( $type_of_widget == 'slider' ) { ?>
								
								<li class="edgtf-tweet-holder">
									<div class="edgtf-tweet-text" <?php aalto_edge_inline_style( $twitter_style ); ?>>
			...
			esc_html_e( 'It seams that you haven\'t connected with your Twitter account', 'edgtf' );
		}
