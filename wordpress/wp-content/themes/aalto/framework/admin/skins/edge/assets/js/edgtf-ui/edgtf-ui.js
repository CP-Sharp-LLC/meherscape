			...
			frame: function() {
				...
				this._frame.on('update', function() {
					...
					jQuery.ajax({
						type: "post",
						url: ajaxurl,
						data: "action=aalto_edge_gallery_upload_get_images&ids=" + ids,
    ...
    */
	function edgtfInitAjaxForm() {
		...
		$(document).delegate(".edgtf_ajax_form", "submit", function (a) {
			var b = $(this);
			var c = {
				action: "aalto_edge_save_options"
			};
			jQuery.ajax({
				url: ajaxurl,
				cache: !1,
			...
			edgtfImportBtn.on('click', function(e) {
				...
				if (confirm(confirmMessage)) {
					...
					if(import_opt == 'content'){
						for( var i=1; i < 10; i++ ){
							var str;
							if (i < 10) str = 'aalto_content_0'+i+'.xml';
							else str = 'aalto_content_'+i+'.xml';
							jQuery.ajax({
								type: 'POST',
								url: ajaxurl,
								data: {
