...
/*
   Class: AaltoEdgeFramework
   A class that initializes Edge Framework
*/
class AaltoEdgeFramework {
	private static $instance;
	public $edgtfOptions;
	public $edgtfMetaBoxes;
	public $edgtfTaxonomyOptions;
	public $edgtfUserOptions;
	private $skin;
	
	private function __construct() {
		$this->edgtfOptions         = AaltoEdgeOptions::get_instance();
		$this->edgtfMetaBoxes       = AaltoEdgeMetaBoxes::get_instance();
		$this->edgtfTaxonomyOptions = AaltoEdgeTaxonomyOptions::get_instance();
		$this->edgtfUserOptions     = AaltoEdgeUserOptions::get_instance();
	}
	...
	public function setSkin( AaltoEdgeSkinAbstract $skinObject ) {
		$this->skin = $skinObject;
	}
}

/**
 * Class AaltoEdgeSkinManager
 *
...
class AaltoEdgeSkinManager {
	...
	/**
	 * @see AaltoEdgeSkinManager::setSkin()
	 */
	...
	private function setSkin( $skinName = 'edge' ) {
		if ( $skinName !== '' ) {
			if ( file_exists( get_template_directory() . '/framework/admin/skins/' . $skinName . '/skin.php' ) ) {
				...
				$skinName = ucfirst( $skinName ) . esc_html__( 'Skin', 'aalto' );
...
/**
 * Class AaltoEdgeSkinAbstract
 *
 * Abstract class that each skin class must extend
 */
abstract class AaltoEdgeSkinAbstract {
	/**
	 * @var string
	 */
	protected $skinName;
	...
	public function loadTemplatePart( $template, $params = array() ) {
		...
		if ( $template !== '' ) {
			include( aalto_edge_find_template_path( 'framework/admin/skins/' . $this->skinName . '/templates/' . $template . '.php' ) );
		}
	}
	...
	public function setShortcodeJSParams() { ?>
		<script>
			window.edgtfSCIcon = '<?php echo aalto_edge_get_skin_uri() . '/assets/img/admin-logo-icon.png'; ?>';
			window.edgtfSCLabel = '<?php echo esc_html( ucfirst( $this->skinName ) ); ?> <?php esc_html_e( 'Shortcodes', 'aalto' ) ?>';
		</script>
	<?php }
...
class AaltoEdgeOptions {
	...
	public function getOptionValue( $key ) {
		...
		if ( array_key_exists( $key, $aalto_edge_options ) ) {
			return $aalto_edge_options[ $key ];
...
/*
   Class: AaltoEdgeAdminPage
   A class that initializes Edge Admin Page
*/
class AaltoEdgeAdminPage implements iAaltoEdgeLayoutNode {
	public $layout;
	private $factory;
	public $slug;
	public $title;
	...
	function __construct( $slug = "", $title = "", $icon = "" ) {
		$this->layout  = array();
		$this->factory = new AaltoEdgeFieldFactory();
		$this->slug    = $slug;
		$this->title   = $title;
		$this->icon    = $icon;
	}
	...
	public function renderChild( iAaltoEdgeRender $child ) {
		$child->render( $this->factory );
	}
}

/*
   Class: AaltoEdgeMetaBoxes
...
/*
   Class: AaltoEdgeMetaBox
   A class that initializes Edge Meta Box
*/
class AaltoEdgeMetaBox implements iAaltoEdgeLayoutNode {
	public $layout;
	private $factory;
	public $scope;
	public $title;
	...
	function __construct( $scope = "", $title = "", $hidden_property = "", $hidden_values = array(), $name = '', $context = 'advanced', $priority = 'high') {
		$this->layout          = array();
		$this->factory         = new AaltoEdgeFieldFactory();
		$this->scope           = $scope;
	...
	public function setTitle( $label ) {
		...
		return $aalto_edge_Framework->getSkin()->getSkinLabel() . ' ' . $label;
	}
}

/*
   Class: AaltoEdgeTaxonomyOptions
   A class that initializes AaltoEdge Taxonomy Options
*/
class AaltoEdgeTaxonomyOptions {
	private static $instance;
...
/*
   Class: AaltoEdgeTaxonomyOption
   A class that initializes AaltoEdge Taxonomy Option
*/
class AaltoEdgeTaxonomyOption implements iAaltoEdgeLayoutNode {
	public $layout;
	private $factory;
	public $scope;
	...
	function __construct( $scope = "" ) {
		$this->layout  = array();
		$this->factory = new AaltoEdgeTaxonomyFieldFactory();
		$this->scope   = $scope;
	}
...
/*
   Class: AaltoEdgeUserOptions
   A class that initializes AaltoEdge User Options
*/
class AaltoEdgeUserOptions {
	private static $instance;
	public $userOptions;
...
/*
   Class: AaltoEdgeUserOption
   A class that initializes AaltoEdge User Option
*/
class AaltoEdgeUserOption implements iAaltoEdgeLayoutNode {
	public $layout;
	private $factory;
	public $scope;
	...
	function __construct( $scope = "" ) {
		$this->layout  = array();
		$this->factory = new AaltoEdgeUserFieldFactory();
		$this->scope   = $scope;
	}
...
if ( ! function_exists( 'aalto_edge_init_framework_variable' ) ) {
	function aalto_edge_init_framework_variable() {
		global $aalto_edge_Framework;
		
		$aalto_edge_Framework = AaltoEdgeFramework::get_instance();
		$edgtfSkinManager                 = new AaltoEdgeSkinManager();
		$aalto_edge_Framework->setSkin( $edgtfSkinManager->getSkin() );
	}
	
	add_action( 'aalto_edge_before_options_map', 'aalto_edge_init_framework_variable' );
}
?>