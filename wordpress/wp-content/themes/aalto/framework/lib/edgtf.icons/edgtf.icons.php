...
/*
  Class: AaltoEdgeIconCollections
...
class AaltoEdgeIconCollections
{
    ...
    private function initIconCollections() {
        $this->addIconCollection('font_awesome', new AaltoEdgeIconsFontAwesome("Font Awesome", "fa_icon"));
        $this->addIconCollection('font_elegant', new AaltoEdgeIconsFontElegant("Font Elegant", "fe_icon"));
        $this->addIconCollection('ion_icons', new AaltoEdgeIonIcons("Ion Icons", "ion_icon"));
        $this->addIconCollection('linea_icons', new AaltoEdgeLineaIcons('Linea Icons', 'linea_icon'));
        $this->addIconCollection('linear_icons', new AaltoEdgeLinearIcons('Linear Icons', 'linear_icon'));
        $this->addIconCollection('simple_line_icons', new AaltoEdgeSimpleLineIcons('Simple Line Icons', 'simple_line_icon'));
        $this->addIconCollection('dripicons', new AaltoEdgeDripicons('Dripicons', 'dripicon'));
    }
    ...
    public function getIconsMetaBoxOrOption($attributes) {
        ...
        $options = array(
            'font_awesome'      => esc_html__('Font Awesome', 'aalto'),
            'font_elegant'      => esc_html__('Font Elegant', 'aalto'),
            'ion_icons'         => esc_html__('Ion Icons', 'aalto'),
            'simple_line_icons' => esc_html__('Simple Line Icons', 'aalto')
        );
        if ($scope == 'regular') {
            $options = array(
                'font_awesome'      => esc_html__('Font Awesome', 'aalto'),
                'font_elegant'      => esc_html__('Font Elegant', 'aalto'),
                'ion_icons'         => esc_html__('Ion Icons', 'aalto'),
                'linea_icons'       => esc_html__('Linea Icons', 'aalto'),
                'linear_icons'      => esc_html__('Linear Icons', 'aalto'),
                'simple_line_icons' => esc_html__('Simple Line Icons', 'aalto'),
                'dripicons'         => esc_html__('Dripicons', 'aalto')
            );
        }
        ...
        foreach ($icon_collections as $collection_key => $collection_object) {
            ...
            $edgtf_icon_pack_container = aalto_edge_add_admin_container(
                array(
                    'parent'          => $parent,
    ...
    public function getVCParamsArray($iconPackDependency = array(), $iconCollectionPrefix = "", $emptyIconPack = false) {
        ...
        $iconPackParams = array(
            'type'        => 'dropdown',
            'heading'     => esc_html__('Icon Pack', 'aalto'),
    ...
    public function getIconWidgetParamsArray() {
        ...
        $iconPackParams[] = array(
            'type'    => 'dropdown',
            'title'   => esc_html__('Icon Pack', 'aalto'),
            'name'    => 'icon_pack',
            'options' => array(
                'font_awesome'      => esc_html__('Font Awesome', 'aalto'),
                'font_elegant'      => esc_html__('Font Elegant', 'aalto'),
    ...
    public function getIconCollectionIcons(iAaltoEdgeIconCollection $collection) {
    ...
    public function getIconCollectionsEmpty($no_empty_key = "") {
        $array = array();
        $array[$no_empty_key] = esc_html__( 'No Icon', 'aalto');
        foreach ($this->iconCollections as $key => $collection) {
            $array[$key] = $collection->title;
        }
    ...
    public function getIconCollectionsVCEmpty() {
        $vc_array = array();
	    $vc_array[ esc_html__( 'No Icon', 'aalto' ) ] = '';
        foreach ($this->iconCollections as $key => $collection) {
            $vc_array[$collection->title] = $key;
        }
...
if (!function_exists('aalto_edge_activate_theme_icons')) {
    function aalto_edge_activate_theme_icons() {
        global $aalto_edge_IconCollections;
        $aalto_edge_IconCollections = AaltoEdgeIconCollections::get_instance();
    }

    add_action('after_setup_theme', 'aalto_edge_activate_theme_icons');
}