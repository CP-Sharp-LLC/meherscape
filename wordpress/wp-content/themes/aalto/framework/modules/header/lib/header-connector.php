...
namespace AaltoEdge\Modules\Header\Lib;
/**
 * Class AaltoEdgeHeaderConnector
 *
...
class AaltoEdgeHeaderConnector {
	...
	public function connect( $config = array() ) {
		do_action( 'aalto_edge_pre_header_connect' );
		...
		if ( ! empty( $config['affect_content'] ) ) {
			add_filter( 'aalto_edge_content_elem_style_attr', array( $this, 'contentMarginFilter' ) );
		}
		
		if ( ! empty( $config['affect_title'] ) ) {
			add_filter( 'aalto_edge_title_content_padding', array( $this, 'titlePaddingFilter' ) );
		}
		
		do_action( 'aalto_edge_after_header_connect' );
	}
	 ...
	 *
	 * Hooks to aalto_edge_title_content_padding filter
	 *
	 * @return int
	 */
