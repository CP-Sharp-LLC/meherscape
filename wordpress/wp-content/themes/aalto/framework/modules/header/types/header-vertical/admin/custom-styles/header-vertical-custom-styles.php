...
if ( ! function_exists( 'aalto_edge_vertical_menu_styles' ) ) {
	function aalto_edge_vertical_menu_styles() {
		...
		$vertical_background_color = aalto_edge_options()->getOptionValue( 'vertical_header_background_color' );
		$vertical_background_image = aalto_edge_options()->getOptionValue( 'vertical_header_background_image' );
		$vertical_shadow_enabled   = aalto_edge_options()->getOptionValue( 'vertical_header_shadow' );
		$vertical_border_enabled   = aalto_edge_options()->getOptionValue( 'vertical_header_border' );
		...
		if ( $vertical_border_enabled === 'yes' ) {
			$header_border_color = aalto_edge_options()->getOptionValue( 'vertical_header_border_color' );
		...
		echo aalto_edge_dynamic_css( $vertical_header_selectors, $vertical_header_styles );
	}
	
	add_action( 'aalto_edge_style_dynamic', 'aalto_edge_vertical_menu_styles' );
}

if ( ! function_exists( 'aalto_edge_vertical_main_menu_styles' ) ) {
	/**
	 * Generates styles for vertical main main menu
	 */
	function aalto_edge_vertical_main_menu_styles() {
		$menu_holder_styles = array();
		
		$menu_top_margin    = aalto_edge_options()->getOptionValue( 'vertical_menu_top_margin' );
		$menu_bottom_margin = aalto_edge_options()->getOptionValue( 'vertical_menu_bottom_margin' );
		
		if ( ! empty( $menu_top_margin ) ) {
			$menu_holder_styles['margin-top'] = aalto_edge_filter_px( $menu_top_margin ) . 'px';
		}
		if ( ! empty( $menu_bottom_margin ) ) {
			$menu_holder_styles['margin-bottom'] = aalto_edge_filter_px( $menu_bottom_margin ) . 'px';
		}
		...
		echo aalto_edge_dynamic_css( $menu_holder_selector, $menu_holder_styles );
		
		// vertical menu 1st level style
		
		$first_level_styles = aalto_edge_get_typography_styles( 'vertical_menu_1st' );
		
		$first_level_selector = array(
			'.edgtf-header-vertical .edgtf-vertical-menu > ul > li > a'
		);
		
		echo aalto_edge_dynamic_css( $first_level_selector, $first_level_styles );
		
		$first_level_hover_styles = array();
		
		$first_level_hover_color = aalto_edge_options()->getOptionValue( 'vertical_menu_1st_hover_color' );
		...
		echo aalto_edge_dynamic_css( $first_level_hover_selector, $first_level_hover_styles );
		
		// vertical menu 2nd level style
		
		$second_level_styles = aalto_edge_get_typography_styles( 'vertical_menu_2nd' );
		
		$second_level_selector = array(
			'.edgtf-header-vertical .edgtf-vertical-menu .second .inner > ul > li > a'
		);
		
		echo aalto_edge_dynamic_css( $second_level_selector, $second_level_styles );
		
		$second_level_hover_styles = array();
		
		$second_level_hover_color = aalto_edge_options()->getOptionValue( 'vertical_menu_2nd_hover_color' );
		...
		echo aalto_edge_dynamic_css( $second_level_hover_selector, $second_level_hover_styles );
		
		// vertical menu 3rd level style
		
		$third_level_styles = aalto_edge_get_typography_styles( 'vertical_menu_3rd' );
		
		$third_level_selector = array(
			'.edgtf-header-vertical .edgtf-vertical-menu .second .inner ul li ul li a'
		);
		
		echo aalto_edge_dynamic_css( $third_level_selector, $third_level_styles );
		
		
		$third_level_hover_styles = array();
		
		$third_level_hover_color = aalto_edge_options()->getOptionValue( 'vertical_menu_3rd_hover_color' );
		...
		echo aalto_edge_dynamic_css( $third_level_hover_selector, $third_level_hover_styles );
	}
	
	add_action( 'aalto_edge_style_dynamic', 'aalto_edge_vertical_main_menu_styles' );
}