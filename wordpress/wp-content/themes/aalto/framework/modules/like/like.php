...
class AaltoEdgeLike {
	...
	private function __construct() {
		add_action( 'wp_ajax_aalto_edge_like', array( $this, 'ajax' ) );
		add_action( 'wp_ajax_nopriv_aalto_edge_like', array( $this, 'ajax' ) );
	}
	...
	public function add_like() {
		...
		$title       = esc_html__( 'Like this', 'aalto' );
		
		if ( isset( $_COOKIE[ 'edgtf-like_' . $post->ID ] ) ) {
			$class = 'edgtf-like liked';
			$title = esc_html__( 'You already like this!', 'aalto' );
		}
...
if ( ! function_exists( 'aalto_edge_create_like' ) ) {
	function aalto_edge_create_like() {
		AaltoEdgeLike::get_instance();
	}
	
	add_action( 'after_setup_theme', 'aalto_edge_create_like' );
}