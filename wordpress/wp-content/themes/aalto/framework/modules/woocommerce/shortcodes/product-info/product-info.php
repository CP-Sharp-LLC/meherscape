...
class ProductInfo implements Lib\ShortcodeInterface {
	...
	public function vcMap() {
		if(function_exists('vc_map')) {
			vc_map(
				array(
					'name'                      => esc_html__( 'Edge Product Info', 'aalto' ),
					'base'                      => $this->getBase(),
					...
					'params'                    => array(
						array(
							...
							'heading'     => esc_html__( 'Selected Product', 'aalto' ),
							'settings'    => array(
								'sortable'      => true,
								'unique_values' => true
							),
							'admin_label' => true,
							'save_always' => true,
							'description' => esc_html__( 'If you left this field empty then product ID will be of the current page', 'aalto' )
						),
						array(
							'type'        => 'dropdown',
							'param_name'  => 'product_info_type',
							'heading'     => esc_html__( 'Product Info Type', 'aalto' ),
							'value'       => array(
								esc_html__( 'Title', 'aalto' )          => 'title',
								esc_html__( 'Featured Image', 'aalto' ) => 'featured_image',
								esc_html__( 'Category', 'aalto' )       => 'category',
								esc_html__( 'Excerpt', 'aalto' )        => 'excerpt',
								esc_html__( 'Price', 'aalto' )          => 'price',
								esc_html__( 'Rating', 'aalto' )         => 'rating',
								esc_html__( 'Add To Cart', 'aalto' )    => 'add_to_cart',
								esc_html__( 'Tag', 'aalto' )            => 'tag',
								esc_html__( 'Author', 'aalto' )         => 'author',
								esc_html__( 'Date', 'aalto' )           => 'date',
							),
							'admin_label' => true
						),
						array(
							...
							'heading'    => esc_html__( 'Product Info Color', 'aalto' ),
						...
						array(
							...
							'description' => esc_html__( 'Set title tag for product title element', 'aalto' ),
						...
						array(
							...
							'heading'    => esc_html__( 'Featured Image Proportions', 'aalto' ),
							'value'      => array(
						...
						array(
							...
							'heading'     => esc_html__( 'Category Tag', 'aalto' ),
							'value'       => array_flip( aalto_edge_get_title_tag( true ) ),
							'description' => esc_html__( 'Set category tag for product category element', 'aalto' ),
						...
						array(
							...
							'heading'    => esc_html__( 'Add To Cart Skin', 'aalto' ),
							'value'      => array(
								esc_html__( 'Default', 'aalto' ) => '',
								esc_html__( 'White', 'aalto' )   => 'white',
						...
						array(
							...
							'heading'    => esc_html__( 'Add To Cart Size', 'aalto' ),
							'value'      => array(
								esc_html__( 'Default', 'aalto' ) => '',
							...
							'dependency' => array( 'element' => 'product_info_type', 'value'   => array( 'aalto' ) ),
							'group'      => esc_html__( 'Product Info Style', 'aalto' )
						)
					)
				)
	...
	public function getItemTitleHtml( $params ) {
		...
		if ( ! empty( $title ) ) {
			$html = '<' . esc_attr( $title_tag ) . ' itemprop="name" class="edgtf-pi-title entry-title" ' . aalto_edge_get_inline_style( $product_info_style ) . '>';
			...
			$html .= '<div class="edgtf-pi-category">';
				foreach ( $categories as $cat ) {
					if ( ! empty( $params['category_tag'] ) ) {
						$html .= '<' . esc_attr( $params['category_tag'] ) . ' ' . aalto_edge_get_inline_style( $product_info_style ) . '>';
						...
						$html .= '<a itemprop="url" class="edgtf-pi-category-item" href="' . esc_url( get_term_link( $cat->term_id ) ) . '" ' . aalto_edge_get_inline_style( $product_info_style ) . '>' . esc_html( $cat->name ) . '</a>';
					}
				}
			$html .= '</div>';
		}
	...
	public function getItemExcerptHtml( $params ) {
		...
		if ( ! empty( $excerpt ) ) {
			$html = '<div class="edgtf-pi-excerpt"><p itemprop="description" class="edgtf-pi-excerpt-item" ' . aalto_edge_get_inline_style( $product_info_style ) . '>' . esc_html( $excerpt ) . '</p></div>';
		}
	...
	public function getItemPriceHtml( $params ) {
		...
		if ( $price_html = $product->get_price_html() ) {
			$html = '<div class="edgtf-pi-price" ' . aalto_edge_get_inline_style( $product_info_style ) . '>' . $price_html . '</div>';
		}
	...
	public function getItemRatingHtml( $params ) {
		...
		if ( get_option( 'woocommerce_enable_review_rating' ) !== 'no' ) {
			...
			$html = '<div class="edgtf-pi-rating" title="' . sprintf( esc_html__( "Rated %s out of 5", "aalto" ), $average ) . '" ' . aalto_edge_get_inline_style( $product_info_style ) . '><span style="width:' . ( ( $average / 5 ) * 100 ) . '%"></span></div>';
		}
	...
	public function getItemAddToCartHtml( $params ) {
		...
		$html .= apply_filters( 'aalto_edge_product_info_add_to_cart_link',
	...
	public function getItemTagHtml( $params ) {
		...
		if ( ! empty( $tags ) ) {
			$html = '<div class="edgtf-pi-tag">';
				foreach ( $tags as $tag ) {
					$html .= '<a itemprop="url" class="edgtf-pi-tag-item" href="' . esc_url( get_term_link( $tag->term_id ) ) . '" ' . aalto_edge_get_inline_style( $product_info_style ) . '>' . esc_html( $tag->name ) . '</a>';
				}
			$html .= '</div>';
		}
	...
	public function getItemAuthorHtml( $params ) {
		...
		if ( ! empty( $author ) ) {
			$html = '<div class="edgtf-pi-author" ' . aalto_edge_get_inline_style( $product_info_style ) . '>' . esc_html( $author ) . '</div>';
		}
	...
	public function getItemDateHtml( $params ) {
		...
		if ( ! empty( $date ) ) {
			$html = '<div class="edgtf-pi-date" ' . aalto_edge_get_inline_style( $product_info_style ) . '>' . esc_html( $date ) . '</div>';
		}
	...
	public function productIdAutocompleteSuggester( $query ) {
		...
		if ( is_array( $post_meta_infos ) && ! empty( $post_meta_infos ) ) {
			foreach ( $post_meta_infos as $value ) {
				$data = array();
				$data['value'] = $value['id'];
				$data['label'] = esc_html__( 'Id', 'aalto' ) . ': ' . $value['id'] . ( ( strlen( $value['title'] ) > 0 ) ? ' - ' . esc_html__( 'Title', 'aalto' ) . ': ' . $value['title'] : '' );
				$results[] = $data;
			}
		}
		...
		if ( ! empty( $query ) ) {
			...
			if ( ! is_wp_error( $product ) ) {
				...
				if ( ! empty( $product_title ) ) {
					$product_title_display = ' - ' . esc_html__( 'Title', 'aalto' ) . ': ' . $product_title;
				}
				
				$product_id_display = esc_html__( 'Id', 'aalto' ) . ': ' . $product_id;
